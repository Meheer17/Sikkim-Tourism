...
async def list_files(skip: int = Query(0, ge=0), limit: int = Query(10, ge=1, le=100), current_user_id: str = Depends(get_current_user_id)):
...
async def download_file(file_id: str, current_user_id: str = Depends(get_current_user_id)):
    file_content, file_name, file_type = await telegram_storage_service.download_file(file_id, current_user_id)
    return StreamingResponse(io.BytesIO(file_content), media_type=file_type, headers={"Content-Disposition": f"attachment; filename={file_name}"})
